...
def scan_repo_impl(repo_url: str, branch: str = "main", commit: str | None = None) -> dict:
    ...
    try:
        ...
        trivy_rc, trivy_out, trivy_err = run_cmd(
            ["trivy","fs","--no-progress","--format","json","--scanners","vuln,secret,misconfig", temp_dir],
            check=False
        )
        ...
        return {
            ...
            "summary": {
                ...
                "gitleaks": {
                    "leaks": len(llm_input["findings"]) - sum(
                        len(r.get("Vulnerabilities") or []) +
                        len(r.get("Misconfigurations") or []) +
                        len(r.get("Secrets") or []) for r in (trivy_json.get("Results") or [])
                    )
                }
            },
            "llm_input": llm_input,
